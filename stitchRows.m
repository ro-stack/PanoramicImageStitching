...
% Run first to create 5 stitched rows
clc;
clear all;
close all;

% For each rows folder add them to the datastore 
for j = 1:5
    
    switch(j)
        case 1
            wallImages = imageDatastore('row1');
        case 2
            wallImages = imageDatastore('row2');
        case 3
            wallImages = imageDatastore('row3');
        case 4
            wallImages = imageDatastore('row4');
        case 5
            wallImages = imageDatastore('row5');
    end
    ...
    % Detect SURF features - also tried HARRIS/MSER/BRISK
    ...
    % Tested with affine too
    numImages = numel(wallImages.Files(1:4));
    ...
    for n = 2:numImages
        ...
        % if statement for only if indexPairs are greater or equal to 4
    ...
    % Create JPEGs for each row in stitchedRows folder
    name = sprintf('stitchedRows/p%.0f.jpg', j);
    imwrite(panorama, name);
